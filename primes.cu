...
bool checkPrimeCpu(long long num) {
    ...
    // any number that can be devided by 2 is not prime
    if (num % 2 == 0) return false;
    // all prime numbers are odd (except for 2)
    // start at 3 and increment by 2 (that way i is always an odd number)
    // i * i is another optimization given divisors come in pairs etc.
    for (long long i = 3; i * i <= num; i += 2) {
        if (num % i == 0) {
            return false;
        }
